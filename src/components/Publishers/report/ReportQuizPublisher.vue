...
export default {
  ...
  methods: {
    ...
    _actionViewDetail(item) {
      if (item.type_start === "REALTIMESOAL") {
        //redirect to realtime soal
        this.$router.push({
          path: "/p/report/detail-quiz-realtime/" + item._id
        });
      } else {
        //redirect to racee ( normal quiz )
        this.$router.push({
          path: "/p/report/detail-quiz/" + item._id
        });
      }
    },
    _actionGetAll() {
