      ...
      <div class="col-md-12">
        ...
        >
          ...
          <template v-slot:cell(report)="data">
            <button class="btn btn-sm btn-success" @click="showReport(data.item)">
...
export default {
  ...
  data() {
    return {
      ...
      isBusy: true,
      value: [],
      pathImages: "https://storage.googleapis.com/edutore-cdn/quiz/",
      attribute: [
        "No",
        {
          key: "judul_quiz",
  ...
  methods: {
    ...
    _actionGetAll() {
      this.isBusy = true;
      this.$axios
        .get(`publisher-quiz?page=${this.currentPage}&limit=${this.perPage}`, {
    ...
    showReport(item) {
      this.$router.push({
        path: "/p/quiz/report-quiz/" + item._id,
        query: { judul_quiz: item.judul_quiz, type_quiz: item.type_quiz }
      });
    },
    asyncFind(query) {
      if (query.length > 3) {
        this.isBusy = true;
        this.$axios
          .get("publisher-quiz/search?judul=" + query, { headers: { "x-access-token": this.token } })
          .then(response => {
            this.items = response.data.data;
    ...
    _actionSearch() {
      this.isBusy = true;
      this.$axios
        .get("publisher-quiz/search?judul=" + this.value.judul_quiz, {
          headers: { "x-access-token": this.token }
        })
        .then(response => {
          this.items = response.data.data;
