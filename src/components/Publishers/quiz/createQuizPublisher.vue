            ...
            <form @submit.prevent="_actionCreate">
              <div class="form-group row">
                ...
                <div class="col-md-9">
                  <ValidationProvider rules="required|min:10" v-slot="{ errors }">
                    <input v-model="Form.judul_quiz" type="text" class="form-control" name="Judul Quiz" />
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-3">
                  <ValidationProvider
                    ...
                    v-slot="{ errors }"
                  >
                    <input id="image" type="file" @change="_changeFile" class="form-control-file" />
                    <span class="text-danger">{{ errors[0] }}</span>
                  </ValidationProvider>
                </div>
                ...
                <div v-if="banner" class="col-sm-2">
                  <span class="btn btn-sm btn-danger" style="cursor:pointer" @click="_actionClearFile">
                    <i class="fa fa-times" />
                  </span>
                </div>
              </div>
              ...
              <div class="form-group row">
                <label for="" class="col-sm-2">
                  Durasi Tampil Soal
                  <br />
                  <small>* Detik (Untuk Type Quiz RealtimeSoal )</small>
                </label>
                <div class="col-sm-2">
      ...
      this.$refs.form.validate().then(success => {
        if (!success) {
          this.$notify({
            group: "foo",
            type: "error",
            title: "Membuat Quiz Gagal!",
            text: "Pastikan form terisi dengan benar."
          });
        } else {
          ...
          FormQuiz.append("is_private", this.Form.is_private);
          ...
          this.$axios
            .post("publisher-quiz", FormQuiz, { headers: { "x-access-token": this.token } })
            .then(response => {
