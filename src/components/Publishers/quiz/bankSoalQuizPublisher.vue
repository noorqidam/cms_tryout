            ...
            <div class="col-sm-12" style="max-height:220px;overflow: scroll;">
              <div class="margin:0;float:left" v-for="p in GroupPublisher" :key="p._id">
                <li style="list-style :none;left:0">
                  <input type="checkbox" v-model="Filterspublishers" :value="p._id" @change="_actionFilter" />
                  {{ p._id }} ({{ p.count }})
                </li>
              </div>
            </div>
              ...
              <div class="col-md-12">
                <div class="form-group row">
                  ...
                  <div class="col-sm-auto">
                    <router-link
                      :to="{ path: '/p/quiz/cart-soal-quiz', query: { _id: quiz_id } }"
                      class="btn btn-sm"
                    >
                      Cart Soal
...
export default {
  ...
  watch: {
    currentPage() {
      if (this.$route.query.matpel || this.$route.query.level) {
        ...
        this._actionQuerySoal(matpel, level);
      } else {
        console.log("get all");
        this._actionGetAll();
      }
  ...
  async mounted() {
    ...
    const { quiz_id } = this.$route.query;
  ...
  methods: {
    ...
    _actionFilter() {
      ...
      const { matpel } = this;
      const { level } = this;
      this._acrtionFilterSoal(Publisher, matpel, level);
    },
    _actionGetAll() {
      this.isBusy = true;
      this.$axios
    ...
    _acrtionFilterSoal(id, matpel, level) {
      this.isBusy = true;
      this.$axios
        .get(
          `publisher-banksoals/filter?_id=${id}&matpel=${matpel}&level=${level}&page=${this.currentPage}&limit=${this.perPage}`,
          { headers: { "x-access-token": this.token } }
        )
        .then(response => {
          this.isBusy = false;
    ...
    _actionQuerySoal(matpel, level) {
      if (matpel) {
        if (matpel && level) {
          this._acrtionFilterSoal(matpel, level);
        }
        this.isBusy = true;
        this.$axios
          .get(
