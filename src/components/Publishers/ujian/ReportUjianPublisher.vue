          ...
          <table class="table table-hover">
            ...
            <tbody v-if="dataPeserta.length > 0">
              <tr v-for="(pesertaUjian, index) in dataPeserta" :key="index">
                ...
                <td v-if="pesertaUjian.kelas == 'No Data'">
                  {{ pesertaUjian.kelas }}
                </td>
                <td v-else>{{ pesertaUjian.kelas }} - {{ pesertaUjian.sub_kelas.toUpperCase() }}</td>
                <td>{{ pesertaUjian.score }}</td>
                <td>
                  <span
            ...
            <tbody v-else>
              <tr class="text-center">
                <td colspan="4">Tidak ada data Peserta</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  mounted() {
    ...
    this.ujian_id = id;
    this.actionGerReportUjian();
  },
  methods: {
    actionGerReportUjian() {
      let data = {
    ...
    _actionDownloadRecap() {
      ...
      if (data.length < 1) {
        ...
        data.forEach(element => {
          let dataPeserta = [++n, element.email, element.score];
