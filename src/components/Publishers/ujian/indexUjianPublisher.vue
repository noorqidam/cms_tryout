        ...
        >
          ...
          <template v-slot:cell(ujian_banner)="data">
            <img
              ...
              :src="pathImages + data.item.ujian_banner"
          ...
          <template v-slot:cell(waktu)="data">
            {{ _actionViewDate(data.item.start_date) }} {{ data.item.times }}
          </template>
          <template v-slot:cell(publish)="data">
            <span v-if="data.item.publish" class="badge badge-success"> Publish </span>
          ...
          <template v-slot:cell(report)="data">
            <button class="btn btn-sm btn-success" @click="reportUjian(data.item)">
...
export default {
  ...
  methods: {
    ...
    _actionGetAll() {
      this.isBusy = true;
      this.$axios
        .get(`publisher-ujian?page=${this.currentPage}&limit=${this.perPage}`, {
    ...
    _actionDelete(id) {
      ...
      this.$axios
        .post("publisher-delete-ujian", data, { headers: { "x-access-token": this.token } })
        .then(response => {
          this.$notify({
            group: "foo",
            title: "Berhasil ",
    ...
    reportUjian(item) {
      this.$router.push({
        path: "/p/ujian/report-ujian-sesi/" + item._id
      });
    },
    asyncFind(query) {
      if (query.length > 3) {
        this.isBusy = true;
        this.$axios
          .get("publisher-ujian/search?judul=" + query, { headers: { "x-access-token": this.token } })
          .then(response => {
            this.items = response.data.data;
    ...
    _actionSearch() {
      this.isBusy = true;
      this.$axios
        .get("publisher-ujian/search?judul=" + this.value.judul_ujian, {
          headers: { "x-access-token": this.token }
        })
        .then(response => {
          this.items = response.data.data;
