...
export default {
  ...
  methods: {
    ...
    actionGetDetail(id) {
      this.$axios
        .get(`live-quiz/${id}`, { headers: { "x-access-token": this.token } })
        .then(response => (this.items = response.data.data))
        .catch(err => {
          console.log(err);
        });
    }
    // actionGetLiveUserTest(id) {
    //   this.$axios
    //     .get(`ratting-quiz?quiz_id=${id}`, { headers: { "x-access-token": this.token } })
    //     .then(response => (this.items = response.data.data))
    //     .catch(err => {
    //       console.log(err);
    //     });
