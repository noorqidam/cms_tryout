    ...
    <div class="col-md-12 mb-2">
      ...
      <div v-for="item in items" :key="item._id" class="col-md-12">
        <div
          ...
          @click="_actionViewDetail(item._id, item.type_start)"
          class="card hover"
          style="margin: 5px"
        >
          <div class="row">
...
export default {
  ...
  methods: {
    ...
    _actionViewDetail(id, type_start) {
      if (type_start == "REALTIMESOAL") {
        this.$router.push({
          path: "/report/detail-quiz-realtime/" + id
        });
      } else if (type_start == "RACE") {
        this.$router.push({
          path: "/report/detail-quiz/" + id
        });
      }
    },
    _actionGetAll() {
