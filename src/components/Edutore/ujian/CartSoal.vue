        ...
        <div class="card-body">
          <form @submit.prevent="_actionStored">
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-8">
                <input v-model="Form.sesi_ujian" type="text" class="form-control" required />
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    _actionStored() {
      ...
      // let newJam = this.Form.duration.HH + ":" + this.Form.duration.mm;
      let data = {
      ...
      console.log(data);
      this.$axios
        .post("soal-ujian", data, { headers: { "x-access-token": this.token } })
        .then(response => {
          this.$notify({
            group: "foo",
            title: "Success Add ",
          ...
          this.deleteStoreDb();
          this.$router.push({
            path: "/ujian/ujian-detail/" + this.id_ujian
          });
        })
        .catch(err => {
          console.log(err);
    ...
    async getDb() {
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = e => {
          console.log("Success Open DB 2 cart ujian");
