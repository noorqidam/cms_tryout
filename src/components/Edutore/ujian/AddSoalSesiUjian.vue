              ...
              <div class="col-md-12">
                <div class="form-group row">
                  ...
                  <div class="col-sm-auto">
                    <router-link
                      :to="{
                        path: '/draft-sesi-soal-ujian',
                        query: { ujian_id: ujian_id, sesi_id: sesi_id }
                      }"
                      class="btn btn-sm"
                    >
                      Cart Soal
...
export default {
  ...
  computed: {
    filteredList() {
      if (this.Filter) {
        return this.GroupMatpel.filter(item => {
          item._id.toLowerCase().indexOf(this.Filter.toLowerCase()) > -1;
        });
      } else {
  ...
  mounted() {
    ...
    let ujian = this.$route.query.ujian_id;
    let sesi = this.$route.query.sesi_id;
    this.ujian_id = ujian;
    this.sesi_id = sesi;
    this._actionGetAll();
  ...
  methods: {
    ...
    _actionStoreSoal() {
      let soal = JSON.stringify(this.selected);
      let checkSoal = localStorage.getItem("soal");
      let newArray = this.selected;
      console.log(newArray);

      // let UniqueSoal = []
      if (checkSoal) {
        let first = JSON.parse(checkSoal);
        const result = [];
        const map = new Map();
        let final = [...first, ...newArray];
        ...
        let dataSoal = JSON.stringify(result);
        localStorage.setItem("soal", dataSoal);
        this.clearSelected();
      } else {
    ...
    async getDb() {
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = e => {
          console.log("Success Open DB at sesi ujian");
